...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7635342394:AAHbGqr8-ZouSg8B3b9eVF6tk5jCSMWdDns'
OWNER_USERNAME = "Davil9089"
CO_OWNERS = []  # List of user IDs for co-owners
OWNER_CONTACT = "Contact @Davil9089 to buy keys"
ALLOWED_GROUP_IDS = [-1002683885721]
MAX_THREADS = 2000
max_duration = 120
bot_open = False
SPECIAL_MAX_DURATION = 300
...
async def open_bot(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        ...
        f"🔑 *For 240 seconds attacks, keys are still required. Buy from {get_display_name(update.effective_chat.id if update.effective_chat.type in ['group', 'supergroup'] else None)}*",
        parse_mode='Markdown'
    )
        ...
        if not is_authorized_user(update):
            await update.message.reply_photo(
                ...
                caption=f"❌ *This bot is not authorized to use here.*\n\n"
                        f"💬 *Need a key? DM:@Mafiaop0781* TALK TO OWNER",
                parse_mode='Markdown'
            )
            return
...
async def attack_input(update: Update, context: CallbackContext):
    ...
    if len(args) != 4:
        ...
        await update.message.reply_text(
            ...
            f"👑 *Bot Owner:* {current_display_name}\n"
            f"💬 *Need a key for 240s? DM:* {current_display_name}",
            parse_mode='Markdown'
        )
        return ConversationHandler.END
    ...
    if duration > max_duration and not is_special:
        ...
        await update.message.reply_text(
            ...
            f"🔑 *For 240 seconds attacks, you need a special key.*\n\n"
            f"👑 *Buy keys from:* {current_display_name}",
            parse_mode='Markdown'
        )
        return ConversationHandler.END
...
async def rules(update: Update, context: CallbackContext):
    ...
    rules_text = (
        ...
        "MY BEST FRIENDS RAJA RIYAAZ DEMON ALONE RAJ RITIK 😘❤️😊.\n\n"
        f"👑 *Bot Owner:* {current_display_name}\n"
